...
async function diagnoseAll() {
  ...
  const promises = appointments.value.map((apt) =>
    diagnosePatient(apt.apt_id, apt.symptom),
  );
          ...
          <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              ...
              <div class="border-t border-slate-800/50 pt-4 mt-auto">
                <div
                  v-if="diagnosisResults[apt.apt_id]"
                  class="bg-gradient-to-r from-emerald-500/10 to-indigo-500/10 border border-emerald-500/30 rounded-xl p-3.5 animate-fadeIn"
                >
                  <span
