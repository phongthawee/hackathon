<template>
  <div class="flex flex-col gap-lg">
    ...
    >
      <div>
        <h1 class="font-display-lg text-display-lg text-on-surface mb-xs">
          ภาพรวมการเฝ้าระวัง
        </h1>
        <p
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-md mb-xs">
      ...
      >
        ...
        <div
          class="font-body-md text-body-md text-secondary flex items-center gap-1 text-sm font-medium"
        >
          <span
      ...
      >
        ...
        <div
          class="font-headline-md text-headline-md text-on-surface mb-1 flex items-center gap-sm"
        >
          <div
            class="w-3 h-3 rounded-full"
            :class="
              kpis.alertLevel === 'RED' ? 'bg-error' : 'bg-tertiary-fixed-dim'
            "
          ></div>
          {{
      ...
      >
        ...
        <div
          class="font-headline-md text-headline-md text-on-surface mb-1 text-base font-bold text-on-tertiary-container"
        >
      ...
      >
        ...
        >
          ...
          >
          -0.5%
          <span class="text-on-surface-variant font-normal">จากค่าเฉลี่ย</span>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-md items-start">
      ...
      <div class="lg:col-span-7 h-full">
        ...
        <ClientOnly>
          <LeafletMap :hotspots="hotspots" @select-city="handleSelectCity" />
        </ClientOnly>
      </div>
      ...
      <div class="lg:col-span-5 flex flex-col gap-md">
        ...
        <div
          ...
          class="bg-surface-container-lowest rounded-xl p-md shadow-sm border border-secondary/40"
        >
          <div
            class="flex justify-between items-center mb-md border-b border-surface-variant pb-xs"
          >
            <h3 class="font-headline-sm text-base text-primary font-bold">
              📊 คลัสเตอร์ระบาด: {{ selectedCityData.city }}
            </h3>
            <button
              class="text-on-surface-variant hover:text-on-surface text-xl font-bold"
              @click="selectedCityData = null"
            >
              ×
            </button>
          </div>
          <div class="flex flex-col gap-sm text-sm">
            <div class="flex justify-between">
              ...
              <span class="font-bold text-on-surface"
                >{{ selectedCityData.totalCases }} ราย</span
              >
            </div>
            <div class="flex justify-between">
              <span class="text-on-surface-variant"
                >เคส Active (กำลังรักษา):</span
              >
              <span class="font-bold text-secondary"
                >{{ selectedCityData.activeCases }} ราย</span
              >
            </div>
            <div class="flex justify-between items-center">
              ...
              <span
                ...
                :class="
                  selectedCityData.severity === 'high'
            ...
            <div class="mt-sm bg-surface-container-low p-sm rounded-lg">
              <div class="font-bold text-xs text-on-surface mb-sm">
                กลุ่มอาการที่พบบ่อยที่สุด:
              </div>
              <ul
                class="list-disc list-inside text-xs text-on-surface-variant flex flex-col gap-1"
              >
                <li
                  v-for="(sym, idx) in selectedCityData.topSymptoms"
                  :key="idx"
                >
                  {{ sym }}
                </li>
            ...
            <button
              @click="investigateCity(selectedCityData.city)"
              class="w-full bg-secondary text-on-secondary py-2 rounded-lg font-label-caps text-xs font-bold hover:opacity-95 mt-sm"
            >
        ...
        <!-- AI Analyst Console -->
        <div
          class="bg-surface-container-lowest rounded-xl p-md shadow-sm border border-surface-variant flex flex-col h-[400px]"
        >
          <div
            class="flex items-center gap-sm mb-md border-b border-surface-variant pb-xs"
          >
            ...
            <div>
              ...
              <p class="text-[10px] text-on-surface-variant">
                Gemini ระบาดวิทยาปัญญาประดิษฐ์
              </p>
            </div>
          </div>
          ...
          <!-- Chat area -->
          <div
            class="flex-1 overflow-y-auto pr-sm flex flex-col gap-sm mb-sm text-xs"
            ref="chatLogsRef"
          >
          ...
          <!-- Chat inputs -->
          <form
            class="flex gap-sm border-t border-surface-variant pt-sm"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-md">
      ...
      <div
        class="lg:col-span-4 bg-surface-container-lowest rounded-xl p-md shadow-sm border border-surface-variant flex flex-col"
      >
        <div
          class="flex items-center justify-between mb-md border-b border-surface-variant pb-sm"
        >
          <h3
          ...
          <span
            class="bg-error-container text-on-error-container font-label-caps text-[10px] px-2 py-1 rounded-full"
            >สด</span
          >
        </div>
        ...
        <!-- Alerts log list -->
        <div
          class="flex-1 flex flex-col gap-sm overflow-y-auto pr-1"
          style="max-height: 300px"
        >
          <!-- Alert Item 1 (Dynamic Live Pings indicator) -->
          <div
            v-if="lastPingText"
            class="p-sm rounded-lg bg-surface-container border border-secondary/40 cursor-pointer animate-pulse"
          >
            <div class="flex justify-between items-start mb-1">
              <div
                class="font-label-caps text-label-caps text-secondary font-bold"
              >
                LIVE PING DETECTED
              </div>
              <div
                class="font-data-mono text-data-mono text-on-surface-variant text-xs"
              >
                เมื่อกี้
              </div>
            </div>
            <div
              class="font-body-md text-body-md text-on-surface font-medium mb-1"
            >
              {{ lastPingText }}
            </div>
            <div
              class="font-body-md text-body-md text-on-surface-variant text-xs flex items-center gap-1"
            >
              <span
                class="material-symbols-outlined text-outline"
                style="font-size: 14px"
          ...
          <!-- Alert Item 2 -->
          <div
            class="p-sm rounded-lg bg-surface hover:bg-surface-variant transition-colors border border-surface-variant cursor-pointer group"
          >
            <div class="flex justify-between items-start mb-1">
              <div class="font-label-caps text-label-caps text-error font-bold">
                แจ้งเตือนด่วน
              </div>
              <div
                class="font-data-mono text-data-mono text-on-surface-variant text-xs"
            ...
            <div
              class="font-body-md text-body-md text-on-surface font-medium mb-1 group-hover:text-secondary transition-colors"
            >
              กลุ่มก้อน: ผื่นไม่ทราบสาเหตุ
            </div>
            <div
              class="font-body-md text-body-md text-on-surface-variant text-xs flex items-center gap-1"
            >
          ...
          >
            <div class="flex justify-between items-start mb-1">
              <div
                class="font-label-caps text-label-caps text-on-tertiary-container font-bold"
              >
                ระดับสูง
              </div>
              <div
          ...
          <!-- Alert Item 4 -->
          <div
            class="p-sm rounded-lg bg-surface hover:bg-surface-variant transition-colors border border-surface-variant cursor-pointer group opacity-70"
          >
            <div class="flex justify-between items-start mb-1">
              <div
            ...
            <div
              class="font-body-md text-body-md text-on-surface font-medium mb-1 group-hover:text-primary transition-colors line-through"
            >
              ตรวจจับ: ระบบล่ม Lab API
            </div>
          </div>
        </div>
        ...
        <NuxtLink
          to="/explorer"
          class="mt-sm w-full py-2 text-center font-label-caps text-label-caps text-primary hover:bg-surface-variant rounded transition-colors text-xs font-bold block border border-surface-variant"
        >
          ดูบันทึกทั้งหมด
        </NuxtLink>
      </div>
  ...
  try {
    ...
    if (statsData.success) {
      ...
      if (selectedCityData.value) {
        const found = statsData.hotspots.find(
          (h) => h.city === selectedCityData.value.city,
        );
    ...
    const casesData = await $fetch("/api/appointments", {
      query: { limit: 400 },
    });
...
function formatMarkdown(text) {
  ...
  return html
    ...
    .replace(
      /`(.*?)`/g,
      '<code class="bg-surface-container px-1 py-0.5 rounded text-[11px]">$1</code>',
    )
    .replace(/\n/g, "<br />");
}
...
function renderMarkdownTable(rows) {
  ...
  let tableHtml =
    '<div class="overflow-x-auto my-3 rounded-xl border border-surface-variant shadow-sm"><table class="w-full text-xs border-collapse text-left"><thead class="bg-surface-container-high font-bold border-b border-surface-variant text-on-surface"><tr>';

  headers.forEach((h) => {
    tableHtml += `<th class="p-3 border-r border-surface-variant last:border-0">${h}</th>`;
  });
  tableHtml +=
    '</tr></thead><tbody class="divide-y divide-surface-variant bg-surface">';
