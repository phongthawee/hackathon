<template>
  <div class="flex min-h-screen w-full bg-background text-on-background font-body-md antialiased overflow-x-hidden">
    ...
    <aside 
      ...
      :class="[
        isMobileOpen ? 'translate-x-0 w-64 px-md' : '-translate-x-full lg:translate-x-0',
    ...
    >
      ...
      <div class="mb-xl px-sm flex items-center justify-between gap-2">
        <div class="flex items-center gap-2 overflow-hidden">
          <span class="material-symbols-outlined text-primary text-[32px] flex-shrink-0">radar</span>
          <span v-show="!isCollapsed" class="font-display-lg text-display-lg font-bold text-primary dark:text-inverse-primary tracking-tight transition-all duration-300">Health Radar</span>
        </div>
        <!-- Collapse Trigger for Desktop -->
        <button @click="isCollapsed = !isCollapsed" class="hidden lg:flex w-8 h-8 rounded-full items-center justify-center text-on-surface-variant hover:bg-surface-variant transition-all flex-shrink-0">
          <span class="material-symbols-outlined text-sm">{{ isCollapsed ? 'chevron_right' : 'chevron_left' }}</span>
        </button>
        <!-- Close button for Mobile drawer -->
        <button @click="isMobileOpen = false" class="lg:hidden w-8 h-8 rounded-full flex items-center justify-center text-on-surface-variant hover:bg-surface-variant flex-shrink-0">
          <span class="material-symbols-outlined">close</span>
        </button>
      </div>

      <!-- User Profile Card -->
      <div class="flex items-center gap-md mb-xl p-sm rounded-lg hover:bg-surface-variant transition-colors duration-200 cursor-pointer border border-transparent hover:border-outline-variant justify-center lg:justify-start" :class="{ 'lg:px-2': isCollapsed }">
        <img alt="Dr. Nan profile picture" class="w-10 h-10 rounded-full object-cover shadow-sm border border-surface-variant flex-shrink-0" src="https://lh3.googleusercontent.com/aida-public/AB6AXuCW2PYUEA3gek5AJPO192PVgT89p676egbhhrsDEXpvwkXGCLgOv9kZ_PsM1LGlBAXqrqhC5HiwAj5gxZngu_REJlplwoPOlNtjvNebpPUL5bF0q4B9LA-NlV6vNlYubGE6-t2NHg1Rsb7LUMbhowd2EjHRXcc7xKn6R6VFniefIoAOyYP90jqS9j42QD79hgyT54mwQW63qOsAfCtZviERyy42KcN3Q0e5MeX8Jv00VouzRfKU_a6OZHFYBvGPYSwNtpi4uDOlY9IY"/>
        <div v-show="!isCollapsed" class="flex flex-col overflow-hidden">
          <span class="font-headline-sm text-body-lg font-semibold text-on-surface truncate">Dr. Nan</span>
          <span class="font-body-md text-label-caps text-on-surface-variant truncate">Regional Epidemiologist</span>
        </div>
      </div>

      <!-- CTA Button for simulation pinger -->
      <button @click="triggerLivePing" class="w-full bg-primary text-on-primary py-sm rounded-lg font-label-caps text-label-caps flex items-center justify-center gap-2 hover:bg-primary/90 transition-colors shadow-sm mb-lg px-2 lg:px-md" :title="isCollapsed ? 'New Alert' : ''">
        <span class="material-symbols-outlined text-[18px] flex-shrink-0">add_alert</span>
      ...
      <nav class="flex-grow space-y-sm flex flex-col">
        <!-- Dashboard -->
        <NuxtLink to="/" class="flex items-center gap-md py-sm rounded-md font-body-md text-body-md transition-colors duration-200 hover:bg-surface-variant text-on-surface-variant justify-center lg:justify-start px-2 lg:px-sm" active-class="text-primary font-bold border-r-4 border-primary bg-primary-container/5" :title="isCollapsed ? 'Dashboard' : ''" exact>
        ...
        <!-- Live Map -->
        <NuxtLink to="/map" class="flex items-center gap-md py-sm rounded-md font-body-md text-body-md transition-colors duration-200 hover:bg-surface-variant text-on-surface-variant justify-center lg:justify-start px-2 lg:px-sm" active-class="text-primary font-bold border-r-4 border-primary bg-primary-container/5" :title="isCollapsed ? 'Live Map' : ''">
          <span class="material-symbols-outlined text-[20px] flex-shrink-0">map</span>
          <span v-show="!isCollapsed" class="truncate">Live Map</span>
        </NuxtLink>

        <!-- Outbreak Risk Map -->
        <NuxtLink to="/outbreaks" class="flex items-center gap-md py-sm rounded-md font-body-md text-body-md transition-colors duration-200 hover:bg-surface-variant text-on-surface-variant justify-center lg:justify-start px-2 lg:px-sm" active-class="text-primary font-bold border-r-4 border-primary bg-primary-container/5" :title="isCollapsed ? 'Outbreak Risk Map' : ''">
          <span class="material-symbols-outlined text-[20px] flex-shrink-0">spatial_tracking</span>
          <span v-show="!isCollapsed" class="truncate">Outbreak Risk Map</span>
        </NuxtLink>

        <!-- Case Explorer -->
        <NuxtLink to="/explorer" class="flex items-center gap-md py-sm rounded-md font-body-md text-body-md transition-colors duration-200 hover:bg-surface-variant text-on-surface-variant justify-center lg:justify-start px-2 lg:px-sm" active-class="text-primary font-bold border-r-4 border-primary bg-primary-container/5" :title="isCollapsed ? 'Case Explorer' : ''">
        ...
        <!-- Reports -->
        <a class="flex items-center gap-md py-sm rounded-md font-body-md text-body-md transition-colors duration-200 hover:bg-surface-variant text-on-surface-variant cursor-not-allowed opacity-60 justify-center lg:justify-start px-2 lg:px-sm" href="#" :title="isCollapsed ? 'Reports' : ''">
          <span class="material-symbols-outlined text-[20px] flex-shrink-0">description</span>
      ...
      <div class="mt-auto pt-lg border-t border-surface-container flex flex-col gap-sm">
        <a class="flex items-center gap-md py-xs rounded-md font-body-md text-body-md text-on-surface-variant hover:bg-surface-variant transition-colors justify-center lg:justify-start px-2 lg:px-sm" href="#" :title="isCollapsed ? 'Settings' : ''">
        ...
        <a class="flex items-center gap-md py-xs rounded-md font-body-md text-body-md text-on-surface-variant hover:bg-surface-variant transition-colors justify-center lg:justify-start px-2 lg:px-sm" href="#" :title="isCollapsed ? 'Support' : ''">
          <span class="material-symbols-outlined text-[18px] flex-shrink-0">help</span>
    ...
    >
      ...
      <header v-if="route.path !== '/map'" class="bg-surface-bright flex justify-between items-center w-full px-lg py-md sticky top-0 z-30 border-b border-surface-variant">
        <div class="flex items-center gap-md w-full lg:w-1/3">
          ...
          <button @click="isMobileOpen = true" class="lg:hidden w-10 h-10 rounded-full flex items-center justify-center text-on-surface-variant hover:bg-surface-container-high transition-all flex-shrink-0">
            <span class="material-symbols-outlined">menu</span>
          </button>
